...
function UsefulLinksDropdown() {
  ...
  return (
    <div className="w-[1100px] drop-shadow-[0px_4px_2px_rgba(0,0,0,0.25)]">
      <button
        onClick={() => setOpen(!open)}
        className="w-full bg-[#00629b] h-[70px] flex items-center justify-between px-7 cursor-pointer"
      >
        <p className="font-['Refrigerator_Deluxe:ExtraBold',sans-serif] text-[35px] text-left text-white tracking-[1.7px] uppercase">Useful Links</p>
        <Chevron open={open} />
      </button>
      {open && (
        <div className="grid grid-cols-5 gap-[2px]">
          {usefulLinks.map((link) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-[#d9d9d9] h-[201px] flex items-center justify-center p-4 hover:bg-[#c9c9c9] transition-colors"
            >
              <p className="font-['Source_Sans_Pro:SemiBold',sans-serif] text-[#1e1e1e] text-[18px] text-center tracking-[0.9px]">{link.label}</p>
            </a>
          ))}
        </div>
      )}
  ...
  return (
    <div className="bg-white min-w-[1440px] min-h-screen flex flex-col items-center">
      ...
      <div className="relative h-[400px] w-[1440px] overflow-hidden flex items-center justify-center">
        ...
        <p className="absolute font-['Refrigerator_Deluxe:ExtraBold',sans-serif] text-[80px] text-white tracking-[4.8px] uppercase text-center w-[880px]">onboarding</p>
      </div>

      {/* Dropdowns */}
      <div className="flex flex-col items-center gap-[72px] py-12 bg-white">
        <UsefulLinksDropdown />
      </div>
